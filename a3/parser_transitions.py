...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':   
            ...
            self.dependencies.append((self.stack[-1], second_top))

        elif transition == 'RA':
            top = self.stack.pop()
            self.dependencies.append((self.stack[-1], top))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Does unfinished_parses = partial_parses work ?  what is the differences ? 
    unfinished_parses = partial_parses[:]
